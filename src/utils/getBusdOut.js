...
export async function getBusdOut(tokenName, tokenAddress, amount){
    if(tokenName === "BUSD") return amount
    var path = [tokenAddress.toLowerCase(),]
    if(tokenAddress.toLowerCase() != BNB) path.push(BNB)
    path.push(BUSD)
    console.log(path)
    let base_amt = new BigNumber(1).shiftedBy(18);
    var out = await getContract(PancakeAbi, "0x10ED43C718714eb63d5aA57B78B54704E256024E".toLowerCase())
    .methods.getAmountsOut(base_amt, path).call()
    console.log(out)
